...
with open("config_uat.json", "r") as config_file:
...
POSTGRES_DB_prod = config_data["postgres_uat"]["db"]
POSTGRES_USER_prod= config_data["postgres_uat"]["user"]
POSTGRES_PASSWORD_prod = config_data["postgres_uat"]["password"]
POSTGRES_HOST_prod = config_data["postgres_uat"]["host"]
POSTGRES_PORT_prod = config_data["postgres_uat"]["port"]

# PostgreSQL Configurations
POSTGRES_DB_uat = config_data["postgres_uat"]["db"]
POSTGRES_USER_uat = config_data["postgres_uat"]["user"]
POSTGRES_PASSWORD_uat = config_data["postgres_uat"]["password"]
POSTGRES_HOST_uat = config_data["postgres_uat"]["host"]
POSTGRES_PORT_uat = config_data["postgres_uat"]["port"]
...
def run_postgres_queries(borrower_gst):
    engine = create_engine(f'postgresql://{POSTGRES_USER_uat}:{POSTGRES_PASSWORD_uat}@{POSTGRES_HOST_uat}:{POSTGRES_PORT_uat}/{POSTGRES_DB_uat}')
...
def transaction_recommendation():
    ...
    try:
        ...
        if query1_df.empty and query2_df.empty:
            print({"error": "No data returned from additional queries."}), 400
    except Exception as e:

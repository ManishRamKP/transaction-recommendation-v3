...
with open("config_prod.json", "r") as config_file:
...
FROM 
   cbapisprd.trade_master AS t
INNER JOIN 
    cbapisprd.finance_request_trade_map AS frtm 
ON 
    t.id = frtm.trademaster_id 
INNER JOIN 
    cbapisprd.finance_request AS fr 
ON 
...
WHERE 
    ...
    SELECT
        ia.*
    FROM
        cbapisprd.individual_assessment AS ia
    INNER JOIN
        cbapisprd.cbcre_process AS cp
    ON
...
def compute_credit_score(borrower_gst, trader_gst, current_invoice_amount, ctin, invoice_id):
    api_url = "http://127.0.0.1:8114/compute_credit_score"
    headers = { 

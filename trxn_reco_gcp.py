...
try:
    with open("config_gcp.json", "r") as config_file:
...
try:
    ...
    GCP_PROJECT_ID = os.environ.get('GCP_PROJECT_ID', config_data['gcp']['project_id'])
...
try:
    POSTGRES_DB = config_data["postgres"]["db"]
    ...
    POSTGRES_PORT = config_data["postgres"]["port"]
except KeyError as e:
    print(f"Missing key in configuration data: {e}")
    exit(1)

# Initialize GCP clients with loaded credentials
storage_client = storage.Client(project=GCP_PROJECT_ID, credentials=GCS_CREDENTIALS)
client = bigquery.Client(project=GCP_PROJECT_ID, credentials=GCS_CREDENTIALS)

# Add more code here to continue the script execution
...
FROM 
   cbapisv2.trade_master AS t
INNER JOIN 
    cbapisv2.finance_request_trade_map AS frtm 
ON 
    t.id = frtm.trademaster_id 
INNER JOIN 
    cbapisv2.finance_request AS fr 
ON 
...
WHERE 
    ...
    SELECT
        ia.*
    FROM
        cbapisv2.individual_assessment AS ia
    INNER JOIN
        cbapisv2.cbcre_process AS cp
    ON
...
def compute_credit_score(borrower_gst, trader_gst, current_invoice_amount, ctin, invoice_id):
    ...
    headers = { 
        'Authorization': 'Bearer ' + os.environ.get('API_BEARER_TOKEN', 'YOUR_BEARER_TOKEN')
    }
    additional_metrics = {
        "grn_present": False,
    ...
    try:
        ...
        # Capture the response status and content
        ...
        if response.status_code == 200:
            ...
            return {
                ...
                "response_content": response_content,  # Return the raw response content
                "response_status": response_status     # Include the status code
            }
        else:
...
def transaction_recommendation():
    ...
    try:
        ...
        return jsonify({"error": f"Failed to execute additional BigQuery queries: {str(e)}"}), 500
...
# Save the response to GCS
    try:
        ...
        blob = bucket.blob(f"transaction_recommendations/{request_id}.json")
        blob.upload_from_string(
            data=json.dumps(response),
            content_type='application/json'
        )
        print(f"Response saved to GCS at {BUCKET_NAME}/transaction_recommendations/{request_id}.json")
    except Exception as e:
        print(f"Failed to save response to GCS: {str(e)}")
...
# Save the response to GCS
    try:
        bucket = storage_client.bucket(BUCKET_NAME)
        blob = bucket.blob(f"transaction_recommendations_refine/{cumulative_assessment['request_id']}.json")
        blob.upload_from_string(
            data=json.dumps(response),
            content_type='application/json'
        )
        print(f"Response saved to GCS at {BUCKET_NAME}/transaction_recommendations_refine/{cumulative_assessment['request_id']}.json")
    except Exception as e:
        print(f"Failed to save response to GCS: {str(e)}")
